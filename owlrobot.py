...
class Motor():
    ...
    # normalized PWM speed -1.0..1.0 for DC motors without velocity feedback
    def setPwmSpeed(self, aSpeed):
        aSpeed = max(-1.0, min(1.0, float(aSpeed)))
        self.speed = aSpeed
        if self.swapDirection:
            aSpeed *= -1.0
        self.robot.sendCanData(OWL_DRIVE_MSG_ID, self.nodeId, can_cmd_set, can_val_pwm_speed, struct.pack('<f', aSpeed))
...
class Robot():
    def __init__(self, aname = "owlRobot"):
        ...
        self.bus = None
        self.notifier = None
        self.lastCanSendErrorAt = 0.0
        if can is None:
            print('python-can not installed; CAN motor output disabled')
        else:
            try:
                print('trying slcan0...')
                if not self.canChannelReady('slcan0'):
                    raise RuntimeError('slcan0 is not up')
                self.bus = can.interface.Bus(channel='slcan0', bustype='socketcan', receive_own_messages=True)
                self.notifier = can.Notifier(self.bus,[self.onCanReceived])
                print('ok')
            except:
                try:
                    print('trying can0...')
                    if not self.canChannelReady('can0'):
                        raise RuntimeError('can0 is not up')
        ...
        self.stopButtonUpdated = 0.0
        self.motorCurrents = {
            LEFT_MOTOR_NODE_ID: 0.0,
            RIGHT_MOTOR_NODE_ID: 0.0,
            TOOL_MOTOR_NODE_ID: 0.0,
        }
        self.ipAddress = ''

    def canChannelReady(self, channel):
        path = f'/sys/class/net/{channel}/operstate'
        if not os.path.exists(path):
            return False
        try:
            return open(path, 'r', encoding='utf-8').read().strip() == 'up'
        except OSError:
            return False
    ...
    def onCanReceived(self, can):
        ...
        if can.arbitration_id == OWL_CONTROL_MSG_ID:
            ...
            if cmd == can_cmd_info and val == can_val_stop_button_state:
                self.stopButtonPressed = byteData != 0
                self.stopButtonUpdated = time.time()
        elif can.arbitration_id == OWL_DRIVE_MSG_ID:
            if cmd == can_cmd_info and val == can_val_total_current:
                if node.sourceId in self.motorCurrents:
                    self.motorCurrents[node.sourceId] = round(floatData, 2)
    ...
    def sendCanData(self, msgId, destNodeId, cmd, val, data):        
        ...
        try:
            ...
            now = time.time()
            if now - self.lastCanSendErrorAt > 5.0:
                print('CAN send failed:', exc)
                self.lastCanSendErrorAt = now
            error_code = getattr(exc, "error_code", None)
            if error_code == 105 or "No buffer space available" in str(exc):
                return
            self.closeCanBus()
    ...
    def requestStopButtonState(self):
        self.sendCanData(OWL_CONTROL_MSG_ID, OWL_CONTROL_NODE_ID, can_cmd_request, can_val_stop_button_state, bytes([]))

    def requestMotorCurrents(self):
        for node_id in (LEFT_MOTOR_NODE_ID, RIGHT_MOTOR_NODE_ID, TOOL_MOTOR_NODE_ID):
            self.sendCanData(OWL_DRIVE_MSG_ID, node_id, can_cmd_request, can_val_total_current, bytes([]))
    ...
    def motorPwmSpeedDifferential(self, leftMotorSpeed, rightMotorSpeed, toolMotorSpeed=0.0):
        leftMotorSpeed = max(-1.0, min(1.0, float(leftMotorSpeed)))
        rightMotorSpeed = max(-1.0, min(1.0, float(rightMotorSpeed)))
        toolMotorSpeed = max(-1.0, min(1.0, float(toolMotorSpeed)))
        self.sendCanData(OWL_DRIVE_MSG_ID, LEFT_MOTOR_NODE_ID, can_cmd_set, can_val_pwm_speed, struct.pack('<f', leftMotorSpeed))
        self.sendCanData(OWL_DRIVE_MSG_ID, RIGHT_MOTOR_NODE_ID, can_cmd_set, can_val_pwm_speed, struct.pack('<f', rightMotorSpeed))
        self.sendCanData(OWL_DRIVE_MSG_ID, TOOL_MOTOR_NODE_ID, can_cmd_set, can_val_pwm_speed, struct.pack('<f', toolMotorSpeed))
